...
from PySide6.QtCore import QFile, QIODevice, QTextStream, QFileInfo, QCoreApplication, Qt
...
def edit_redo(main_window):
    pressEvent = QKeyEvent(QKeyEvent.KeyPress, Qt.Key_Z, Qt.ControlModifier | Qt.ShiftModifier)
    releaseEvent = QKeyEvent(QKeyEvent.KeyRelease, Qt.Key_Z, Qt.ControlModifier | Qt.ShiftModifier)
    tab = main_window.ui.editor_win.currentWidget()
    QCoreApplication.postEvent(tab, pressEvent)
    QCoreApplication.postEvent(tab, releaseEvent)
    ...
    if filePath is None:
        ...
        for idx in range(main_window.ui.editor_win.count()):
            if 'Untitled' in main_window.ui.editor_win.tabText(idx):
                try:
                    mangler = max(
                        mangler, int(main_window.ui.editor_win.tabText(idx).split('-')[-1]))
                    mangler += 1
                except ValueError:
                    mangler += 1
...
def file_openFile(main_window, filePath=None):
    if filePath is None:
        filePath = QFileDialog.getOpenFileName(main_window, 'Open file...', os.getcwd())[0]
        if filePath == '':
            return
...
def file_openFolder(main_window):
    dirPath = QFileDialog.getExistingDirectory(main_window, 'Open folder...', os.getcwd())
    if dirPath == '':
        return
