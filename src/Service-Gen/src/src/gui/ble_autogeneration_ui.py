...
class App(Tk):
    def __init__(self):
        ...
        self.wrapper_frame = LabelFrame(
            self,
            background="#FFFFFF",
            relief="flat",
            highlightthickness=1,
            highlightbackground=MXM_GREEN,
            highlightcolor=MXM_GREEN,
        )
        ...
        # label
        self.excel_file_path = MXMBrowseWidget(
            self.wrapper_frame,
            browse_name="BLE Excel File Folder",
            browse_type=MXMBrowseWidget.FILE_BROWSE,
            tooltip="BLE service definition xlsx file",
        )
        self.excel_file_path.grid(row=0, column=1, padx=5, pady=5, sticky="W")
        self.include_path_browse = MXMBrowseWidget(
            self.wrapper_frame,
            browse_name="Include Folder",
            tooltip="BLE autogenerated header files folder",
        )
        self.include_path_browse.grid(row=1, column=1, padx=5, pady=5, sticky="W")
        self.source_path_browse = MXMBrowseWidget(
            self.wrapper_frame,
            browse_name="Source Folder",
            tooltip="BLE autogenerated source files folder",
        )
        self.source_path_browse.grid(row=2, column=1, padx=5, pady=5, sticky="W")
        self.relative_header = MXMLabeledEntry(
            self.wrapper_frame,
            browse_name="Include File Relative Path",
    ...
    def generate(self):
        ...
        execute_ble_renderer(
            self.excel_file_path.get_selected_path(),
