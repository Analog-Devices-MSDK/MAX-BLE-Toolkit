...
def update_discovered_devices(main_window, device):
    ...
    if main_window.ui.logAll.isChecked():
        ...
        try:
            ...
            if (
                device_name
                == main_window.ui.list_widget_discovered.currentItem().text()
            ):
                add_device_to_table()
                update_rssi_graph()
...
def add_table_item(main_window, data):
    ...
    main_window.ui.table_adv_data.setHorizontalHeaderItem(
        0, QTableWidgetItem("Advertised Data")
    ...
    main_window.ui.table_adv_data.setItem(
        rowPosition, 0, item
    )  # Place the data in the first column

    main_window.ui.table_adv_data.setColumnWidth(
        0, 700
    )  # Set the width to your desired value
    main_window.ui.table_adv_data.setRowHeight(
        rowPosition, 40
...
def init_signals_and_slots(main_window):
    main_window.ble_scanner.discovered_devices.connect(
        lambda device: update_discovered_devices(main_window, device)
    )
    main_window.ble_scanner.scanning_stoped.connect(
        lambda: timed_scanning_stoped(main_window)
    )
    main_window.ui.txt_scan_filter.textChanged.connect(
        lambda: txt_scan_filter_changed(main_window)
    )
