...
def setup_tabs(main_window, createStarter=True):
    ...
    main_window.ui.editor_win.tabBar().setTabButton(0, QTabBar.RightSide, main_window.ui.newTab_btn)
    main_window.ui.editor_win.setTabEnabled(0, False)

    main_window.ui.editor_win.setCornerWidget(main_window.ui.runCode_btn, Qt.TopRightCorner)
...
def close_editorTab(main_window, idx):
    ...
    if tab.saveNeeded:
        if tab.filePath is None and tab.toPlainText() == '':
            ...
            res = main_window.showSaveNeededDialog(fname=main_window.ui.editor_win.tabText(idx))
