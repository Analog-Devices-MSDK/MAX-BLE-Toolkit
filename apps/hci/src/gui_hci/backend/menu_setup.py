...
def setup_menubarFile(main_window):
    main_window.ui.actionNew_file.triggered.connect(lambda: file_createNewFile(main_window))
    main_window.ui.actionOpen_File.triggered.connect(lambda: file_openFile(main_window))
    main_window.ui.actionOpen_Folder.triggered.connect(lambda: file_openFolder(main_window))
    ...
    main_window.ui.actionClose_folder.triggered.connect(lambda: file_closeFolder(main_window))
...
def createFiletreeContextMenu(main_window, pos):
    ...
    if main_window.ui.filetree.topLevelItemCount() > 0:
        ...
        if selected_item is None:
            ...
            newFile_action.triggered.connect(lambda: nav_createNewFile(main_window, selected_item))
            newFolder_action = QAction("New Folder", main_window.ui.filetree)
            newFolder_action.triggered.connect(lambda: nav_createNewFolder(main_window, selected_item))
            ...
            delete_action.triggered.connect(lambda: nav_delete(main_window, selected_item))
            rename_action = QAction("Rename", main_window.ui.filetree)
            rename_action.triggered.connect(lambda: nav_rename(main_window, selected_item))
            context.addAction(newFile_action)
            context.addAction(newFolder_action)
            ...
            openFile_action.triggered.connect(lambda: nav_openFile(main_window, selected_item))
            delete_action = QAction("Delete", main_window.ui.filetree)
            delete_action.triggered.connect(lambda: nav_delete(main_window, selected_item))
            rename_action = QAction("Rename", main_window.ui.filetree)
            rename_action.triggered.connect(lambda: nav_rename(main_window, selected_item))
