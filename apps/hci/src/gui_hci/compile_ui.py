...
FNAME_IN2OUT = {"main.ui": "ui_main.py", "main_alwaysRef.ui": "ui_mainAlwaysRef.py"}
...
if __name__ == "__main__":
        ...
        for idx, line in enumerate(codeLines):
            if not codeStarted:
                if line.startswith("class"):
                    codeStarted = True
                    codeLines.insert(
                        idx - 1, "from .nav_window import QDirectoryTree\n"
                    )
                    codeLines.insert(
                        idx - 1, "from .ref_window import QDeselectableTreeView\n"
                    )
                continue
    ...
    if compile_refView:
        subprocess.run(
            ["pyside6-uic", "main_alwaysRef.ui", "-o", "backend/ui_mainAlwaysRef.py"]
        )
