...
def render_ble_service_common(
    ...
    output = template.render(
        service_info=service_info, df=df, ble_util=UtilsBLE, include_path=include_path
    )
...
def render_ble_service_source(
    service_info, df, output_path_c="outputs/", include_relative_path="./"
):
    filename = service_info["SheetName"].replace(" ", "_")
    render_ble_service_common(
        filename,
        service_info,
        df,
        template_file="bleGenerationTemplate_Source.j2",
        output_extension=".c",
        output_path=output_path_c,
        include_path=include_relative_path,
    )
...
def render_ble_service_header(service_info, df, output_path_h="outputs/"):
    ...
    render_ble_service_common(
        ...
        service_info,
        df,
        template_file="bleGenerationTemplate_Header.j2",
        output_extension=".h",
...
def render_ble_service_dart(service_info, df, output_path="outputs/"):
    ...
    render_ble_service_common(
        ...
        service_info,
        df,
        template_file="bleGenerationTemplate_Dart.j2",
        output_extension=".dart",
        output_path=output_path,
    )


def execute_ble_renderer(
    filepath,
    output_path_h="outputs/",
    output_path_c="outputs/",
    ...
    for index, each_service in df_services.iterrows():
        ...
        df["Group"] = df["Group"].fillna(
            "NoGroup"
        )  ## Workaround to change to a string! instead of handling NaN
        render_ble_service_source(
            service_info=each_service,
            df=df,
            output_path_c=output_path_c,
            include_relative_path=include_relative_path,
        )
        render_ble_service_header(
            service_info=each_service, df=df, output_path_h=output_path_h
        )
